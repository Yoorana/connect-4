    ...
    * First number is row, second number is column number
    ...
    makeButtonsForColumns(navElement) {
        for(let boxIndex = 0; boxIndex < this.board.columns; boxIndex++){
            navElement.innerHTML+=`<button class="btColumnSelection" id="column_${boxIndex}" onmouseover="game.hoverColour(this.id)" onmouseout="game.defaultColour(this.id)"` +
     ...
     */
    hoverColour(id) {
        document.getElementById(id).style.backgroundColor = this.game.players[this.game.turn - 1].colour;
    }
    /**
    ...
    spawnDisk(column) {
        for(let row = this.boxesData[column].length - 1; row > - 1; row--){
            ...
            if(!this.boxesData[column][row]) {
                ...
                this.checkWinner(this.winColumn(column), this.winRow(column, row), this.winDiagonal(column, row));
    ...
    winRow(inputColumn, inputRow) {
        ...
        for(let column = 0; column <= this.boxesData[inputColumn].length; column++) {
     ...
     */
     winDiagonal(inputColumn, inputRow) {
        ...
        } catch (e){ // "e" stands for the error object
    ...
    enterPlayerNames() {
     ...
     /**
      * Temporarly hides the elements with a certain classname using CSS
